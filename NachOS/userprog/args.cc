...
SaveArgs(int address)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        for(int z = 0 ;machine->ReadMem(address + i * 4, 4, &strAddr) == false && z<=4; z++ );
        ReadStringFromUser(strAddr, args[i], MAX_ARG_LENGTH);
        DEBUG('X', "ReadStringFromUser arg[i] = %s , i = %d .\n",args[i],i);
    }
...
WriteArgs(char **args)
{
    ...
    for (unsigned i = 0; i < c; i++){
        for( int z = 0 ;machine->WriteMem(sp + 4 * i, 4, argsAddress[i])==false && z<=4 ;z++ );
    }
    for (int z = 0;machine->WriteMem(sp + 4 * c, 4, 0)==false && z<=4; z++);  // The last is null.
    sp -=16;
