...
SyscallHandler(ExceptionType _et)
{
    ...
    switch (scid) {
        ...
        case SC_CREATE: { //Hacer que se aborten las syscall en caso de error
            ...
            if(fileSystem->Create(filename,1000)) machine -> WriteRegister(2, -1);
            break;
        }
        case SC_OPEN:{
            int filenameAddr = machine->ReadRegister(4); //a0
        ...
        case SC_EXEC:{
            ...
            char filename[FILE_NAME_MAX_LEN + 1];
            DEBUG('e',"fileAddr %d, filename %s , size %d ",filenameAddr,filename,sizeof filename);
            if (!ReadStringFromUser(filenameAddr, filename, sizeof filename)){
                DEBUG('e', "Error: filename string too long (maximum is %u bytes).\n",
                      FILE_NAME_MAX_LEN);
                machine -> WriteRegister(2, -1);
