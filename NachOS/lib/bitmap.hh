...
typedef struct _filedata{
    ...
    int writers; //cantida de escritories
    bool deleted; //ya lo borro alguien?
    Condition * read;
    Condition * write;
    Lock * lock;
    List <Thread *> * uthreads; // Lista de threads con el archivo abierto
...
public:
    ...
    ///
    /// * `nitems` is the number of items in the bitmap.
    Syncmap(unsigned nitems);

    /// Uninitialize a bitmap.
    ~Syncmap();

    /// Set the “nth” bit.
    void Mark(unsigned which, int r , int w);
    //marca el archivo con la flag de borrado
    void MarkDelete(unsigned sector);

    /// Clear the “nth” bit.
    void Clear(unsigned which, int r , int w);
    ...
    filedata* FileCheck(unsigned sector);
        List <filedata *> *record;
    void PrintAll() const;

private:
    Bitmap * bmap;
 //cuando seteo un bitmap (relativo al sector) guardos los datos ahi
    Lock * lock;
