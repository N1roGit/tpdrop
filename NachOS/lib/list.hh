...
public:
    ...
    //apply function to item with key
    void HasModify(int key,int readers,int writers ,void (*func)(Item,int,int));
    Item KeyPop(int key);
    Item KeyCheck(int key);
...
template <class Item>
void
List<Item>::HasModify( int key,int readers,int writers ,void (*func)(Item,int,int)) 
{
    ASSERT(func != nullptr);
    for (ListNode *ptr = first; ptr != nullptr; ptr = ptr->next) {
        if (key == ptr->key) {
            DEBUG('f', "Encontre el sector en la lista.\n");
            func(ptr->item,readers,writers);
            return;
        }
    }
}
...
template <class Item>
bool
List<Item>::HasKey(int key) const
{
    for (ListNode *ptr = first; ptr != nullptr; ptr = ptr->next) {
        if (key == ptr->key) {
            return true;
...
List<Item>::PrintL(){
    for (ListNode *ptr = first; ptr != nullptr; ptr = ptr->next) {
            printf("Pid %d - Nombre %s | ",ptr->key,ptr->item->GetName());
    }
    printf("\n");
}
