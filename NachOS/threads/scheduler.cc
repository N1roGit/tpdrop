...
Scheduler::Scheduler()
{
    for(int i = 0; i < max_priority; i++) readyList[i] = new List<Thread *>;
}

/// De-allocate the list of ready threads.
Scheduler::~Scheduler()
{
   for(int i = 0; i < max_priority; i++) delete readyList[i];
}
...
Scheduler::ReadyToRun(Thread *thread)
{
    ...
    ASSERT(thread->get_priority()>=0 && thread->get_priority()<max_priority);
    readyList[thread->get_priority()]->Append(thread);
...
void
Scheduler::Promote(Thread *thread, int prioridad_nueva)
{
    readyList[thread->get_priority()]->Remove(thread);

    thread->set_priority(prioridad_nueva);
    ReadyToRun(thread);
}
