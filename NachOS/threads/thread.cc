...
Thread::Thread(const char *threadName, bool join, unsigned int priority)
{
    name     = threadName;
    stackTop = nullptr;
    stack    = nullptr;
...
Thread::Finish(int ret)
{
    ...
    if( currentThread->can_join ) (currentThread->canal)->Send(ret);
    ASSERT(this == currentThread);
...
Thread::Join(){
    ...
    ASSERT(can_join);
    int msg;
    DEBUG('t', "Thread %s hizo un join\n", GetName());
    canal->Receive(&msg);
    DEBUG('t', "Thread %s salio de un join\n", GetName());
    return msg;
}


unsigned int Thread::get_priority(){
    return prioridad;
}
...
Table <OpenFile*>* Thread::get_files(){
    return openfiles;
}

int Thread::FileAdd(OpenFile* open){
    int i = openfiles->Add(open);
    if(i<0) return i;
    else return i+2; //guardo lugar para el 0 y 1 que son la consola
}

OpenFile* Thread::FileGet(int id){

   return openfiles->Get(id-2);
}

bool Thread::FileHasKey(int i){
    return openfiles->HasKey(i-2);
    
}

bool Thread::FileIsEmpty(){
    return openfiles->IsEmpty();

}
OpenFile* Thread::FileRemove(int i){
    return openfiles->Remove(i-2);
}
