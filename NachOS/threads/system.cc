...
SynchConsole* consola_sinc;   /// declaro la consola global sincro
Table<Thread*> *spaceIds; // Tabla de threads por pid
//Bitmap *bitMap; /// bitmap de paginacion
Coremap *coremap; //tabla de core , que incluye bitmap
#endif
...
TimerInterruptHandler(void *dummy)
{
    if (interrupt->GetStatus() != IDLE_MODE)
        DEBUG('Y',"This is Tick number %d and it is time to YIELD\n",stats->totalTicks);
...
Initialize(int argc, char **argv)
{
    ...
    currentThread = new Thread("main",true,4);
    ...
    consola_sinc = new SynchConsole();
    if (!randomYield) //si no uso -rs
    ...
    syncmap = new Syncmap(NUM_SECTORS);//numero de sectores
#endif
