...
public:
    ...
    /// Initialize a `Thread`.
    Thread(const char *debugName, bool join,unsigned int priority);
    ...
    /// The thread is done executing. y le manda el status al padre (si tiene)
    void Finish(int ret);
    ...
    Table <OpenFile*> * get_files();

    int FileAdd(OpenFile* open);
    OpenFile* FileGet(int id);
    bool FileHasKey(int i);
    bool FileIsEmpty();
    OpenFile* FileRemove(int i);
    SpaceId pid; //Spaceid es un int
    int active_reads;
    int active_writes;
