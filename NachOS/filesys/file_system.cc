...
FileSystem::FileSystem(bool format)
{
    ...
    DEBUG('f', "Zeroing the disk.\n");
    char* zero = new char[SECTOR_SIZE]();
    for (unsigned i = 0; i < NUM_SECTORS; i++)
        synchDisk->WriteSector(i, zero);
    delete zero;

        Bitmap     *freeMap = new Bitmap(NUM_SECTORS);//NUMSECTORS
        Directory  *dir     = new Directory(NUM_DIR_ENTRIES);
        FileHeader *mapH    = new FileHeader(FREE_MAP_SECTOR);
...
FileSystem::Create(const char *name, unsigned initialSize)
{
    ...
    if (dir->Find(name) != -1){
        ...
        if (sector == -1)
            ...
            FileHeader *h = new FileHeader(sector);
...
FileSystem::Open(const char *name)
{
    ...
    //lo abre solo si es valido y no esta borrado pero en uso o si no esta en uso pero no borrado
    DEBUG('f'," %d \n",syncmap->FileCheck(sector)==nullptr);
    if (sector >= 0 && ((syncmap->FileCheck(sector)!=nullptr  &&  (!syncmap->FileCheck(sector)->deleted)) ||  syncmap->FileCheck(sector)==nullptr )){
        openFile = new OpenFile(sector);  // `name` was found in directory.
        DEBUG('f', "El archivo %s fue abierto.\n",name);
    }
    delete dir;
    DEBUG('f', "Open has open %p .\n", openFile);
...
FileSystem::Remove(const char *name)
{
    ...
    FileHeader *fileH = new FileHeader(sector);
    fileH->FetchFrom(sector);

    Bitmap *freeMap = new Bitmap(NUM_SECTORS);
    freeMap->FetchFrom(freeMapFile);

    // el ultimo que cierra borra el archivo, los demas lo marcan como borrado
    //borra si  no es null, y no tiene threads con el archivo abierto o si es null
    DEBUG('F', "Marco sector %d como borrado.\n",sector);
    syncmap->MarkDelete(sector);

    if(((syncmap->FileCheck(sector)!=nullptr &&  (syncmap->FileCheck(sector)->uthreads->IsEmpty())) ||  syncmap->FileCheck(sector)==nullptr)){
        DEBUG('F', "%s Borro el sector %d .\n",currentThread->GetName(),sector);
        ...
        DEBUG('F', "%s No pudo liberar el archivo %s pues tiene lectores.\n",currentThread->GetName(),name);    
    }
        return true;
