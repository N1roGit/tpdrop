...
char *
Directory::ReverseFind(unsigned sector)
{
    ASSERT(sector < raw.tableSize);

    for (unsigned i = 0; i < raw.tableSize; i++)
        if (raw.table[i].inUse && raw.table[i].sector == sector)
        {
            //printf("Encontre en la busqueda reversa a %s\n", raw.table[i].name);
            return raw.table[i].name;
        }
    return nullptr;  // sector not in directory

    //printf("Encontre en la busqueda reversa a %s\n", raw.table[sector].name);
    //return raw.table[sector].name;
}
...
Directory::List() const
{
    for (unsigned i = 0; i < raw.tableSize; i++)
        if (raw.table[i].inUse)
            printf("Tableindex:%d S:%d-F:%s\n",i, raw.table[i].sector, raw.table[i].name);
}
...
Directory::Print() const
{
    ...
    for (unsigned i = 0; i < raw.tableSize; i++)
        if (raw.table[i].inUse) {
            ...
            FileHeader *hdr = new FileHeader(raw.table[i].sector);
            hdr->FetchFrom(raw.table[i].sector); //lo podria borrar total ya fetchea el constructor
            hdr->Print(nullptr);
            delete hdr;
        }
    printf("\n");
