...
static unsigned
ReadLine(char *buffer, unsigned size, OpenFileId input,int* status)
{
    // TODO: how to make sure that `buffer` is not `NULL`?

    unsigned i = 0;
    Read(&buffer[i], 1, input);
    if (buffer[0] == PARALLEL_CHAR){
        *status = 1; 
    }
    else{
        *status = 0;
...
main(void)
{
    ...
    for (;;) {
        WritePrompt(OUTPUT);
        int backend = 0;
        const unsigned lineSize = ReadLine(line, MAX_LINE_SIZE, INPUT,&backend);
        if (lineSize == 0)
            continue;
